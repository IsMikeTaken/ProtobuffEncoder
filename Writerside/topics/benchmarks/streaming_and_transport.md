...
> [!NOTE]
> Batch processing of delimited messages is highly efficient, with `ReadDelimited` being slightly faster than `WriteDelimited` due to optimized stream reading patterns.
...
> [!TIP]
> The overhead added by the duplex transport layer is minimal (~100-200 ns per message) compared to raw serialization, making it suitable for low-latency bidirectional communication.
