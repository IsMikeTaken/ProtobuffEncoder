...
> [!TIP]
> `StaticMessage` provides a ~20% performance boost by bypassing the `ContractResolver` dictionary lookup and using pre-compiled delegates for field access.
...
> [!NOTE]
> Validation is extremely fast (~10ns). Invalid messages are validated even faster because the pipeline short-circuits on the first failure.
...
> [!TIP]
> The `ContractResolver` adds minimal overhead once a type is resolved, as it uses a `ConcurrentDictionary` for O(1) retrieval of pre-computed type metadata.
